...
fn get_all_names(docker_event: &DockerEvent) -> Vec<String> {
    let mut names = vec![];
...
impl DockerMonitor {
    pub fn new(docker: Arc<Docker>, authority_wrapper: AuthorityWrapper, domain: String) -> Self {
        Self {
            authority_wrapper,
            docker,
            domain,
    ...
    pub async fn listener(&self, mut receiver: Receiver<DockerEvent>, token: &CancellationToken) {
        loop {
